...
```{r}
data.frame(c("Prom likes","Prom dislikes", "Prom comentarios"), c(mean(mx_dataset$likes),mean(mx_dataset$dislikes),mean(mx_dataset$comment_count)))
```
...
Otro análisis interesante es identificar a que pais pertenece cada uno de los canales del top 10. A continuación se genera un gráfico circular que muestra, en porcentajes, los paises que estan presentes en el top 10.

```{r}
test <- group_by(top_10_global, country) |>
  summarize(sum_videos = sum(Freq), country = first(country))

test <- group_by(test, country) |>
  mutate(percentage = sum(sum_videos)/sum(test$sum_videos)) |>
  mutate(label = scales::percent(percentage))

ggplot(test, aes(x = "", y = percentage, fill = country)) +
  geom_col() +
...
Como muestra el gráfico, solo 3 países están en este top 10, siendo México el que tiene una mayoría importante respecto de Japón y Estados Unidos.
